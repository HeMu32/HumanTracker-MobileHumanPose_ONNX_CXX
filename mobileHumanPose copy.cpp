...
std::tuple<cv::Mat, cv::Mat, cv::Mat, cv::Mat> MobileHumanPose::processOutput(const cv::Mat& output, float abs_depth, const cv::Vec4i& bbox)
{
    ...
    std::cout << "模型输出形状: [" << batch_size << ", " << channels << ", " 
              << height << ", " << width << "]" << std::endl;
    
    // 更新输出参数以匹配实际输出
    output_height = height;
    output_width = width;
    output_depth = channels / joint_num;
    
    std::cout << "调整后的输出参数: joint_num=" << joint_num 
              << ", output_depth=" << output_depth 
              << ", output_height=" << output_height 
              << ", output_width=" << output_width << std::endl;
    ...
    for (int i = 0; i < joint_num; i++) {
        // 创建与输出尺寸匹配的热图
        cv::Mat joint_heatmap(output_depth, output_height * output_width, CV_32F, cv::Scalar(0));
        
        for (int d = 0; d < output_depth; d++) {
            int row_index = i * output_depth + d;
            
            // 检查行索引是否在有效范围内
            if (row_index < heatmaps.rows) {
                cv::Mat src_row = heatmaps.row(row_index);
                cv::Mat dst_row = joint_heatmap.row(d);
                
                // 确保源和目标的大小匹配
                if (src_row.cols == dst_row.cols) {
                    src_row.copyTo(dst_row);
                } else {
                    std::cout << "警告：行 " << row_index << " 的大小不匹配。源: " 
                              << src_row.cols << "，目标: " << dst_row.cols << std::endl;
                    
                    // 调整源矩阵大小以匹配目标矩阵
                    cv::Mat resized_src;
                    resized_src.create(1, dst_row.cols, CV_32F);
                    
                    // 使用最近邻插值调整大小
                    for (int j = 0; j < dst_row.cols; j++) {
                        int src_idx = j * src_row.cols / dst_row.cols;
                        if (src_idx < src_row.cols) {
                            resized_src.at<float>(0, j) = src_row.at<float>(0, src_idx);
                        } else {
                            resized_src.at<float>(0, j) = 0;
                        }
                    }
                    
                    resized_src.copyTo(dst_row);
                }
            } else {
                std::cout << "警告：行索引 " << row_index << " 超出范围，最大行数: " 
                          << heatmaps.rows << std::endl;
                // 这一行保持为零
            }
        }
...
void MobileHumanPose::getModelInputDetails()
{
        ...
        std::cerr << "获取输入详情时出错: " << e.what() << std::endl;
    // 默认值
    channels = 3;
    input_height = 256;
    input_width = 256;
...
void MobileHumanPose::getModelOutputDetails()
{
        ...
        std::cerr << "获取输出详情时出错: " << e.what() << std::endl;
        // 使用默认值
        output_depth = 64 / joint_num;
        output_height = 32;
        output_width = 32;
    }
}
